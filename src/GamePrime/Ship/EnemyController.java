...
public class EnemyController extends Component {
    ...
    public void Start() {
        ...
        ItemSoundProp = gm.Sm.new PlayProp(
                "Sound" + File.separator + "SFX" + File.separator + (String) SFX.get("ItemCreate"), null);
        DestroySoundProp = gm.Sm.new PlayProp(
                "Sound" + File.separator + "SFX" + File.separator + (String) SFX.get("Destroyed"), null);
        DestroySoundProp2 = gm.Sm.new PlayProp(
                "Sound" + File.separator + "SFX" + File.separator + (String) SFX.get("Bomb"), null);
        Shotdelay = 2;
        enemynum = 0;
        ...
        case 1:
            Level1(3, 4);
            BgmProp = gm.Sm.new PlayProp("Sound" + File.separator + "BGM" + File.separator + (String) BGM.get("Level1"),
                    "BGM");
            BgmProp.count = -1;
        ...
        case 2:
            Level2(1, 2);
            BgmProp = gm.Sm.new PlayProp("Sound" + File.separator + "BGM" + File.separator + (String) BGM.get("Level2"),
                    "BGM");
            BgmProp.count = -1;
            gm.Sm.StopClip("BGM", 1);
            gm.Sm.playSound(BgmProp);
        ...
        case 3:
            Level2(4, 4);
            BgmProp = gm.Sm.new PlayProp("Sound" + File.separator + "BGM" + File.separator + (String) BGM.get("Level3"),
                    "BGM");
            BgmProp.count = -1;
            gm.Sm.StopClip("BGM", 1);
            gm.Sm.playSound(BgmProp);
    ...
    public void CheckCollsion(Bullet bullet) {
        ...
        for (ArrayList<Enemy> eList : EnemyPool) {
            ...
            while (iterator.hasNext()) {
                ...
                if ((MaxX >= bulletMinX && MinX <= bulletMaxX) && (MaxY >= bulletMinY && MinY <= bulletMaxY)) {
                    Collsion(e, bullet);
                    if (e.life == 0) {
                        iterator.remove();
                    }
