    ...
    private void drawSettingDetail() {
        ...
        if (itemCode < 2) {
            ...
            graphic.fillRect(gm.frame.getWidth() / 2, gm.frame.getHeight() / 4 + matrix.getHeight() / 8 * 12,
                    (int) (gm.frame.getWidth() / 4 * gm.Sm.GetVolumePercent() / 100), matrix.getHeight());
            graphic.drawString(Integer.toString((int) gm.Sm.GetVolumePercent()),
                    gm.frame.getWidth() / 4 * 3 + matrix.stringWidth("A") * 2,
                    gm.frame.getHeight() / 4 + matrix.getHeight() * 2);
            if (itemCode == 1 && Itemselected)
                graphic.setColor(Color.GREEN);
            else
                graphic.setColor(Color.WHITE);
            if (gm.Sm.IsMute()) {
                graphic.drawString("OFF", gm.frame.getWidth() / 10 * 7 - matrix.stringWidth("OFF") / 2,
                        gm.frame.getHeight() / 4 + matrix.getHeight() * 4);
            } else {
                graphic.drawString("ON", gm.frame.getWidth() / 10 * 7 - matrix.stringWidth("ON") / 2,
                        gm.frame.getHeight() / 4 + matrix.getHeight() * 4);
            }
        }
        if (itemCode == 2 || itemCode == 3) {
            for (int i = 0; i < KeyDefine.KeyFunc.length; i++) {
                ...
                graphic.drawString(KeyDefine.KeyFunc[i],
                        (int) (gm.frame.getWidth() * 6 / 10.0f) - matrix.stringWidth(KeyDefine.KeyFunc[i]),
                        gm.frame.getHeight() / 4 + matrix.getHeight() * (2 * i + 1));
            }
        }
    }
    ...
    public void drawSetting() {
        ...
        for (int i = 0; i < 4; i++) {
            if (itemCode == i) {
                graphic.setColor(Color.GREEN);
                if (Itemselected) {
                    graphic.drawString("*", gm.frame.getWidth() / 5 - matrix.stringWidth(string[i]) / 2 - 16,
                            gm.frame.getHeight() / 4 + matrix.getHeight() * 2 * (i + 1));
                }
            } else {
                graphic.setColor(Color.WHITE);
            }
            graphic.drawString(string[i], gm.frame.getWidth() / 5 - matrix.stringWidth(string[i]) / 2,
                    gm.frame.getHeight() / 4 + matrix.getHeight() * 2 * (i + 1));
        }
        graphic.setColor(Color.GREEN);
        graphic.drawLine(gm.frame.getWidth() / 5 * 2 - 1, gm.frame.getHeight() / 4, gm.frame.getWidth() / 5 * 2 - 1,
                gm.frame.getHeight() / 10 * 9);
        graphic.drawLine(gm.frame.getWidth() / 5 * 2, gm.frame.getHeight() / 4, gm.frame.getWidth() / 5 * 2,
                gm.frame.getHeight() / 10 * 9);
    }
}