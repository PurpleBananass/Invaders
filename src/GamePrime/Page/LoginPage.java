...
public class LoginPage implements GManager {
    ...
    private void Draw() {
        ...
        grpahics.drawString(s, gm.frame.getWidth() / 2
                - fontmatrix.stringWidth(s) / 2, gm.frame.getHeight() / 4 + fontmatrix.getHeight() * 3);
        // 3 letters name.
        int positionX = gm.frame.getWidth()
                / 2
                - (fontmatrix.getWidths()[name[0]]
                        + fontmatrix.getWidths()[name[1]]
                        + fontmatrix.getWidths()[name[2]]
                        + fontmatrix.getWidths()[' ']) / 2;
        ...
        for (int i = 0; i < 3; i++) {
            ...
            positionX = i == 0 ? positionX
                    : positionX
                            + (fontmatrix.getWidths()[name[i - 1]]
                                    + fontmatrix.getWidths()[' ']) / 2;
            grpahics.drawString(Character.toString(name[i]),
                    positionX,
                    gm.frame.getHeight() / 4 + fontmatrix.getHeight()
                            * 6);
        }
    }
    public void Exit(){};
