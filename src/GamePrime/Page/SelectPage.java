...
public class SelectPage implements GManager {
    ...
    void Draw() {
        ...
        grpahics.drawString(selectString, gm.frame.getWidth() / 2 - fontmatrix.stringWidth(selectString) / 2,
                gm.frame.getHeight() / 8);
        if (SelectIndex == 0)
            grpahics.setColor(Color.GREEN);
        else
            grpahics.setColor(Color.WHITE);
        grpahics.drawString("Player", gm.frame.getWidth() / 5 - fontmatrix.stringWidth("Player") / 2,
                gm.frame.getHeight() / 8 * 3);
        if (SelectIndex == 1)
            grpahics.setColor(Color.GREEN);
        else
            grpahics.setColor(Color.WHITE);
        grpahics.drawString("Hard Mode", gm.frame.getWidth() / 5 - fontmatrix.stringWidth("Player") / 2,
                gm.frame.getHeight() / 8 * 5);
        if (PlayMode == 0)
            grpahics.setColor(Color.GREEN);
        else
            grpahics.setColor(Color.WHITE);
        grpahics.drawString("1P", gm.frame.getWidth() / 10 * 6 - fontmatrix.stringWidth("1P") / 2,
                gm.frame.getHeight() / 8 * 3 + fontmatrix.getHeight() * 2);
        if (PlayMode == 1)
            grpahics.setColor(Color.GREEN);
        else
            grpahics.setColor(Color.WHITE);
        grpahics.drawString("2P", gm.frame.getWidth() / 10 * 8 - fontmatrix.stringWidth("2P") / 2,
                gm.frame.getHeight() / 8 * 3 + fontmatrix.getHeight() * 2);
        grpahics.setColor(Color.WHITE);
        grpahics.drawString("ON", gm.frame.getWidth() / 10 * 6 - fontmatrix.stringWidth("ON") / 2,
                gm.frame.getHeight() / 8 * 5 + fontmatrix.getHeight() * 2);
        grpahics.drawString("OFF", gm.frame.getWidth() / 10 * 8 - fontmatrix.stringWidth("OFF") / 2,
                gm.frame.getHeight() / 8 * 5 + fontmatrix.getHeight() * 2);
        grpahics.setColor(Color.GREEN);
        if (HardMode)
            grpahics.drawString("ON", gm.frame.getWidth() / 10 * 6 - fontmatrix.stringWidth("ON") / 2,
